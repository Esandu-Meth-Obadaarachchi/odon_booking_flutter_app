...
The Flutter web build is deployed to **Netlify** at `https://odon-booking.netlify.app` (Netlify project `odon-booking`, site ID `13590ee1-74ba-4d86-b395-0e4b88eff7d8`).

- `netlify.toml` (repo root) sets `publish = build/web` and an SPA redirect (`/*` → `/index.html`).
- **Manual deploy**:

  ```bash
  flutter build web --release
  netlify deploy --prod --dir build/web --site 13590ee1-74ba-4d86-b395-0e4b88eff7d8
  ```

- **Auto-deploy**: `.github/workflows/deploy.yml` builds and deploys on every push to `main` (other branches are ignored). It needs the GitHub repo secrets `NETLIFY_AUTH_TOKEN` and `NETLIFY_SITE_ID`, and only runs once the workflow file is on `main`.

### Backend (Railway)

The backend runs on Railway; `api_service.dart` `baseUrl` points to its URL. The MongoDB URI is currently hardcoded in `server.js`, but the WhatsApp credentials are read from environment variables set in the Railway dashboard. `server.js` calls `require('dotenv').config()`, so a local `.env` works for development — see `flutter_mongodb_backend/.env.example`.
...
The hotel logo lives at `assets/logo.png` (converted from the original `assets/logo.JPG`), declared under `assets:` in `pubspec.yaml`.

- **Launcher icons** (Android + iOS) are generated by the `flutter_launcher_icons` dev dependency, configured via the `flutter_launcher_icons:` block in `pubspec.yaml`. Regenerate after changing the logo with `dart run flutter_launcher_icons`.
- **Web favicon & PWA icons** — `web/favicon.png` and `web/icons/Icon-*.png` are the logo; it shows as the browser tab icon.
...
| `flutter_launcher_icons` (dev)        | Generates Android/iOS launcher icons from the logo |
...
- **Web / iOS download**: `file_saver_web.dart` triggers the PDF download via an anchor element with the `download` attribute — NOT `window.open()`. iOS Safari blocks `window.open()` as a popup when it runs after the async PDF generation, so the invoice would never appear.

## Create Booking from Invoice

After an invoice PDF is generated, `generate_invoice_screen.dart` shows an "Invoice Ready" dialog (`_showPostInvoiceActions`) with follow-up actions:

- **Add as a booking** — opens `RoomSelectionScreen` (the New Booking screen) pre-filled from the invoice. `RoomSelectionScreen` accepts optional `prefill*` constructor params (`prefillGuestName`, `prefillGuestPhone`, `prefillCheckIn`, `prefillCheckOut`, `prefillPackage`, `prefillMealStart`, `prefillNeedDriver`, `prefillTotal`, `prefillAdvance`, `prefillExtraDetails`) applied in `_applyPrefill()`. Room selection is always left to the user; an amber hint banner (the `roomHint` param) shows the invoice's room mix.
- Invoice package names differ from booking package names — `_mapPackageToBooking` translates them (`Bed and Breakfast` → `BnB`, `Room + Dinner` → `Dinner Only`, others unchanged).

## WhatsApp Invoice Sending

The "Send to guest on WhatsApp" action in the post-invoice dialog sends the invoice PDF to the guest via the **WhatsApp Business Cloud API**.

- **Frontend**: `invoice.generateInvoice()` returns an `InvoiceResult` (PDF bytes + filename). `ApiService.sendInvoiceWhatsApp()` POSTs the base64-encoded PDF to the backend.
- **Backend route**: `POST /invoices/send-whatsapp` — uploads the PDF to Meta's media endpoint, then sends a template message with the PDF as the document header. The `bodyParser.json` limit is raised to `15mb` to accept the base64 PDF.
- **Env vars** (set on Railway / `.env`): `WHATSAPP_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_TEMPLATE_NAME` (default `invoice_send`), `WHATSAPP_TEMPLATE_LANG` (default `en_US`), `WHATSAPP_DEFAULT_COUNTRY_CODE` (default `94`).
- **Template requirement**: the approved Meta template must have a **Document header** and a body with **exactly one variable** (`{{1}}` = guest name). A structural mismatch fails every send.
- Phone numbers are normalised server-side by `normalizeWhatsAppPhone` (digits only; local `0xxxxxxxxx` numbers get the country code prepended).
- Degrades gracefully — if the env vars are unset the route returns a clear 500 and the app shows an error snackbar; invoice generation and "Add as a booking" are unaffected.

## WhatsApp Chatbot

The hotel's WhatsApp number can auto-reply to guest messages with an AI agent.

- **Module**: `flutter_mongodb_backend/whatsapp_bot.js` — a Claude-powered (`claude-opus-4-7`) agent using the `@anthropic-ai/sdk` with a manual tool-use loop.
- **Webhook routes** in `server.js`: `GET /whatsapp/webhook` (Meta verification handshake) and `POST /whatsapp/webhook` (incoming messages — acknowledged with `200` immediately, then processed asynchronously).
- **Tools**: `check_room_availability` (queries the `Booking` + `RoomConfig` collections), `get_room_prices` (queries `PriceConfig`), `send_media` (sends photos/PDF over the Cloud API), `request_human_handoff` (flags the conversation for staff).
- **Conversation memory**: stored per guest phone in the `whatsappconversations` collection — last 20 turns, plain text turns only (tool round-trips run in-memory and are not persisted).
- **Hotel knowledge base**: the `HOTEL_INFO` constant in `whatsapp_bot.js`. Edit it to keep answers accurate — the bot hands off to staff rather than inventing facts.
- **Media**: files in `flutter_mongodb_backend/media/`, served at `<PUBLIC_BASE_URL>/media/<file>`, mapped by the `MEDIA` object in `whatsapp_bot.js`.
- **Env vars**: `ANTHROPIC_API_KEY`, `WHATSAPP_VERIFY_TOKEN`, `PUBLIC_BASE_URL` — plus the shared `WHATSAPP_TOKEN` / `WHATSAPP_PHONE_NUMBER_ID`. Set them in the Meta app's webhook config (`WHATSAPP_VERIFY_TOKEN`) and Railway Variables.
- The static system prompt + tool definitions carry `cache_control` for prompt caching. Replies to a guest who messaged first are free-form (no template needed — within WhatsApp's 24-hour service window). Sending invoices and the chatbot share one WhatsApp number with no conflict (sending uses the Cloud API; the chatbot uses the webhook).
